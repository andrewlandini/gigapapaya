...
const MODIFIER_PROMPTS: Record<string, string> = {
  ...
  'Remove Text': `Regenerate this exact frame with all text removed. Any subtitles, captions, watermarks, credits, titles, logos, burned-in timecode, lower thirds, or overlay text of any kind — gone. Paint them out cleanly. The area behind the text should be filled in with what would naturally be there — continuous background, environment detail, skin, fabric, whatever the text was covering. No smudging, no blur patches, no obvious clone artifacts. The result should look like the text was never there — a clean frame straight from the camera with no post overlays.`,
};
